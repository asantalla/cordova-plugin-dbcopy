...
public class sqlDB extends CordovaPlugin {
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException { 		
		...
		else if (action.equalsIgnoreCase("isDatabaseCreated")) {
			this.isDatabaseCreated(args.getString(0), callbackContext);
			return true;
		}
		else {
	...
	private void isDatabaseCreated(String dbName, final CallbackContext callbackContext) {
		final File dbpath;
		dbname = dbName;
		final DatabaseHelper dbhelper = new DatabaseHelper(this.cordova.getActivity().getApplicationContext());
		dbpath = this.cordova.getActivity().getDatabasePath(dbname);
